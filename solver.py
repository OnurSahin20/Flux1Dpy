...
spec = {"soil_model":soil_model_type,"diagonal":tridiagonal_type, "root_model":root_type,
        ...
        'hnew':float64[:],'dt_ini':float64,'dt_new':float64,'stat':int32,'dz':float64}


@jitclass(spec)
class NumericSolver:
    def __init__(self,soil_model,diagonal,root_model,sim_time,sim_temp,dz,ini_head,flux,transp,pond_max):
        ...
        self.dt_min = 1 / 60 # 1 second minimum time step!
        self.dt_max = self.sim_temp
        self.dt_ini = 1.5
        self.dt_new = 1.5
        self.stat = 1
        self.dz = dz
    ...
    def IterateTime(self,hold,flux,tp,dt,pond):
        eps,mass_error = np.power(10,-4),10 # accuracy and initializing the mass error!
        self.hnew[:] = hold[:]
        for i in range(0,15,1):
            self.hnew[0] = self.hnew[1] # free drainage boundary condition will fix after!!!
            hx = self.diagonal.get_TriDiagonal(dt,hold,self.hnew,tp,pond,flux)
            mass_error = self.model.get_error(self.hnew,hx)
            self.hnew[:] = hx[:]   
            if (mass_error<=eps):
                 self.stat = 0
                 break
    ...
    def RunSolver(self):
        ...
        while (count_time>self.sim_time):
            hnew = self.IterateTime(self.ini_head,self.flux[index],self.transp[index],self.dt_ini,pond)
            if self.stat != 0:
                self.dt_ini = self.dt_ini / 2 
            else:
                count_time += self.dt_ini
                pond = self.soil_model.calculate_pond(self.dz,hnew,self.dt_ini,pond,self.flux[index])
                self.dt_ini = self.dt_new
