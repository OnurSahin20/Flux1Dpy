...
class CreateTriDiagonal:
    ...
    def set_top(self,dt,flux_top,head_top,pond,sink):
        ...
        if self.top_bound == 4: # atmospheric boundary
            if (self.head[self.n_1] < self.ha) and (flux_top>=0): # lower boundary
                self.B[self.n_1] =1; self.A[self.n_2] = 0; self.F[self.n_1] = 0
            ...
            if (self.top_bound  == 0) or (self.top_bound == 2): # constant or variable head boundary!
                self.B[self.n_1] =1; self.A[self.n_2] = 0; self.F[self.n_1] = 0
    ...
    def get_new(self,dt,h1,h2,tp,pond,head_top,head_bot,flux_top,flux_bot):
        ...
        for i in range(1,self.n-1):
            ...
            self.A[i-1] = -k1 / self.dz2
            self.B[i] = ((k1 + k2) / self.dz2 + self.cap[i] / dt)
            self.C[i] = -k2 / self.dz2
            self.F[i] = (self.s1[i] - self.s2[i]) / dt + k2 * (self.head[i + 1] - self.head[i]) / self.dz2 - k1 * (
                        self.head[i] - self.head[i - 1]) / self.dz2+ (k2 - k1) / self.dz - sink[i]
